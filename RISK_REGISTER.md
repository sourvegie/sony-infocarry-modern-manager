...
at `ae35ec2`. P16-001 host preparation is now on its task branch and is not
yet a legacy operation approval.
...
an R3-reviewed three-timestamp native legacy evidence procedure. It stops at
`READY_FOR_HARDWARE_TEST` and requires the exact owner phrase
`APPROVE P16-001 LEGACY MIXED TXT BMP CAPTURE 01`; no legacy send or modern
transaction is authorized yet. The native BMP wrapper, mixed record
construction, and mixed-package post-state remain unproven.
