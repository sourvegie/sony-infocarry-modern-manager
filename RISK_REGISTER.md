...
at `ae35ec2`. P16-001 host preparation and Capture 01 intake are on the
task branch. The complete post-operation raw backup is now preserved and
reconciled outside Git; the native gate remains
`BLOCKED_BY_EXTERNAL_EVIDENCE` only for the still-unresolved timestamp mapping,
Manager result/owner observation, operation-specific capacity semantics, and
decoded numeric completion.
...
`BLOCKED_BY_EXTERNAL_EVIDENCE`: the complete post-operation backup now exactly
matches the native transaction model and verifies the mixed post-state,
payloads, shared-file preservation, and fixed-state objects. Timestamp
mapping, Manager result/owner observation, operation-specific capacity
semantics, and decoded completion remain unresolved. No modern mixed
