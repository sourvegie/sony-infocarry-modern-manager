...
| P18-039 bounded generalized flat host profile: one new root-level folder with 1–8 ordered direct TXT/BMP leaves | Existing strict TXT/BMP preparation, exact 3-/4-leaf proofs, and pre-existing conservative host envelope; no five-leaf native proof | `generalized-flat-root-folder-txt-bmp-v1`; deterministic source order, unique CP932-safe names, direct-only paths, 1 MiB per-child and 4 MiB source/1 MiB prepared aggregate host limits, no-conflict plan, canonical candidate/readiness/authorization/readback seams | None for generalized shapes; no physical device operation in P18-039 | Host admission, review, and read-only preflight only; `host_reviewed_not_live_proven`; normal GUI/CLI send remains disabled for this profile | Independent fresh review and owner-authorized physical validation of a representative five-leaf mixed folder; device capacity and generalized recovery remain unproven | Review exact code-bearing head, then run a separate approved hardware validation; do not infer device maxima from these host bounds |
...
| Nesting outside the host/offline profile, multiple packages, batch, unsupported types, duplicate/conflicting names, over-bound flat selections, or existing targets | No sufficiently specific native evidence for this scope | Rejected before authorization/claim/marker/sender; no overwrite, merge, replacement, deletion, restore, V10, firmware, or arbitrary package path | Not proven for transfer | Unavailable / preview-only with a precise reason | Missing scoped evidence and safety model; no automatic grouping | Keep outside P18-039; obtain a separately scoped task |
...
labeled **Experimental** for live execution. The P18 product integration keeps
the exact flat TXT → BMP → TXT and TXT → BMP → TXT → TXT rows as the only
physically proven live shapes; P18-039 additionally admits the bounded
generalized profile for host preparation, review, and preflight without
physical enablement. Unsupported items remain preview-only.
