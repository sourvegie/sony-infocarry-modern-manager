...
Current I.4/I.5/I.6 status: the constrained offline/fake-only gate and
...
| R3 | Arbitrary new files require proven metadata/state construction, capacity handling, and guarded execution. | Critical | Use clean capture 04 as the golden legacy add fixture and capture 7 as the exact folder/package golden fixture. Require exact or documented normalized offline equivalence, unrelated-byte preservation, fail-closed capacity, exact operation binding, one-shot failure behavior, and full read-back before any approved live modern package smoke. | Milestone F is complete offline, and one explicitly approved Milestone G root-level-TXT smoke passed with exact binding, `0x0000` completion, and independent full read-back. Capture 7 adds exactly one root folder and one TXT child in a legacy fixture; its range-5/range-8 candidate exactly matches the post-add blob, with unchanged shared payloads and observed sidecars. I.5 static analysis verifies the native total model limit; the package candidate and authorization bind explicit total-limit fields. I.6 attempt 02 completed one approved constrained folder/TXT smoke with `0x0000`, exact candidate read-back, preserved shared records/payloads/timestamps, and fixed-state equality after the offline verifier correction in `cf7803b`; the suite is **360 tests**. Manager UI accounting, `0x0024`, and unknown `field_14_be32` remain excluded from free-capacity claims. Interrupted-write recovery, arbitrary folders/children, and normal GUI/CLI package action remain blocked. See the preserved result analysis. |
| R4 | The active source tree is in a hidden Codex project mirror while the intended `${LOCAL_USER_ROOT}/Projects/.../modern-client` location is empty. Work could be lost, duplicated, or continued from the wrong copy. | Critical | Establish one version-controlled canonical source tree under the intended Projects directory; copy rather than move, verify hashes/tests, document evidence locations, then mark all other copies as mirrors. | Mitigated on 2026-08-22. Canonical tree initialized with commits `e6cd6c9` and `85b78a9`; source mirror and evidence preserved. See `analysis/canonical-source-migration.md`. |
...
| R16 | Proprietary evidence, credentials, or private device data could be accidentally published through a source remote or a complete Git-history push. | Critical | Before every remote checkpoint, audit all tracked files and `git rev-list --objects --all`; reject original Sony software, ISO content, device backups, USB captures, raw live evidence, credentials, private information, generated output, and temporary artifacts. Keep the remote private only as an additional source/history backup, never as the sole evidence backup. Never rewrite or push a blocked history without explicit owner approval. | Open. The 2026-08-23 audit found no private-key or token signatures, but the tracked and historical `analysis/phase-8-candidate-1/range-*.bin` SnoopyPro-derived fixture (introduced by `85b78a9`, including `range-08.bin` at 2,063,724 bytes) blocks remote creation/push until an owner-approved history-remediation decision. `gh` is not installed, so authentication is also unverified. |
...
6. Retain both completed constrained-write and narrow new-TXT live-smoke
   ...
   live hardware write.
7. Treat R11–R13 during later Library/Prepare integration. R10 remains outside
