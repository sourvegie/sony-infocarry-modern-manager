...
planning remains deferred. The current portable suite is **512 tests** with three
...
| R3 | Arbitrary new files require proven metadata/state construction, capacity handling, and guarded execution. | Critical | Use clean capture 04 as the golden legacy add fixture and capture 7 as the exact folder/package golden fixture. Require exact or documented normalized offline equivalence, unrelated-byte preservation, fail-closed capacity, exact operation binding, one-shot failure behavior, and full read-back before any approved live modern package smoke. | I.6 completed one approved constrained folder/TXT smoke with `0x0000`, exact candidate read-back, preserved shared records/payloads/timestamps, and fixed-state equality after offline verifier correction. Capture 01 verifies the exact four-child TXT structure, ordered payloads, relationships, capacity fit, fixed state, and native range-to-post-blob equality. The owner-supplied timestamp mapping and Project Lead policy now reconcile the modern candidate: existing timestamps are preserved and one explicit timestamp is assigned to all six new records. The native legacy shared timestamp rewrite and child-4 one-second difference remain observed, while the native numeric request-4 word remains explicitly unresolved. Candidate, exact authorization binding, fake-only workflow, and independent read-back pass; the constrained dossier is `READY_FOR_HARDWARE_TEST`. No modern transaction is authorized or performed. I.8, I.9, and I.10 remain offline-only; arbitrary packages, generalized deletion, normal GUI/CLI package action, and interrupted-write recovery remain blocked. |
